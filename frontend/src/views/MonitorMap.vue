...
<template>
  <div class="page">
    ...
    <div class="table-card">
      <div class="table-header">监测点位详情 <span class="table-count">共 {{ points.length }} 个点位</span></div>
      <el-table :data="points" stripe max-height="480">
        <el-table-column prop="name" label="名称" min-width="180" show-overflow-tooltip />
        <el-table-column label="类型" min-width="90">
          <template #default="{ row }">
            <el-tag :type="row.type==='POLLUTION'?'danger':row.type==='CURRENT'?'warning':row.type==='TIDE'?'info':'success'">
        ...
        <el-table-column label="状态" min-width="90">
          <template #default="{ row }">
            <el-tag :type="row.status==='ACTIVE'?'success':row.status==='MAINTENANCE'?'warning':'info'" effect="plain">
...
.table-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; font-size: 15px; font-weight: 600; color: #1e293b; border-bottom: 1px solid #eaf0f6; }
.table-count { font-size: 13px; font-weight: 400; color: #94a3b8; }
.table-card :deep(.el-table th) { background: #f8fafc; color: #475569; font-weight: 600; }
.table-card :deep(.el-table td) { padding: 12px 0; }
</style>
