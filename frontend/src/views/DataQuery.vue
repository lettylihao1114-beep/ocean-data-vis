  ...
  <div class="page">
    ...
    <div class="table-card">
      <el-table :data="list" v-loading="loading" stripe>
        <el-table-column prop="dataTime" label="观测时间" min-width="160" />
        <el-table-column prop="seaArea" label="海域" min-width="100" />
        <el-table-column prop="longitude" label="经度" width="85" />
        ...
        <el-table-column prop="salinity" label="盐度" min-width="90">
          <template #default="{ row }">
            <span class="data-val">{{ row.salinity }} <small>PSU</small></span>
          </template>
        </el-table-column>
        <el-table-column prop="oxygen" label="溶氧" min-width="90">
          <template #default="{ row }">
            <span class="data-val">{{ row.oxygen }} <small>mg/L</small></span>
          </template>
        </el-table-column>
        <el-table-column prop="ph" label="pH" width="75" />
        <el-table-column prop="currentSpeed" label="洋流速度" min-width="100">
          <template #default="{ row }">
            <span class="data-val">{{ row.currentSpeed }} <small>m/s</small></span>
          </template>
        </el-table-column>
        <el-table-column prop="currentDir" label="洋流方向" min-width="100" />
        <el-table-column prop="tideLevel" label="潮汐" min-width="90">
          <template #default="{ row }">
...
.table-footer { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-top: 1px solid #f1f5f9; gap: 16px; flex-wrap: wrap; }
.table-total { font-size: 13px; color: #64748b; flex-shrink: 0; }
